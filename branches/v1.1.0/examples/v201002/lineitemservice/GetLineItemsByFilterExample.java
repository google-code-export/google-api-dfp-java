...
/**
 * This example gets all line items for the given order. The filter
 * retrieves up to the maximum page size limit of 500. To create line items,
 * run CreateLineItemsExample.java.
 */
public class GetLineItemsByFilterExample {
  public static void main(String[] args) {
    try {
      ...
      // Create a filter to only select line items from a given order.
      Filter filter = new Filter("WHERE orderId = '" + orderId + "' LIMIT 500");

      // Get line items by filter.
     LineItemPage page = lineItemService.getLineItemsByFilter(filter);
